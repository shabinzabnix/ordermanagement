...
export default function CRMDashboardPage() {
  ...
  useEffect(() => {
    if (['STORE_STAFF','STORE_MANAGER'].includes(user?.role) && user?.store_id && !form.store_id) {
  ...
  const handleAddCustomer = async (e) => {
    ...
    try {
      ...
      setAddOpen(false);
      setForm({ mobile_number: '', customer_name: '', gender: '', age: '', address: '', store_id: ['STORE_STAFF','STORE_MANAGER'].includes(user?.role) ? String(user?.store_id || '') : '', customer_type: 'walkin' });
      setPage(1);
